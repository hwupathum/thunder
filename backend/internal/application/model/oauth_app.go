...
type OAuthAppConfigDTO struct {
	...
	SigningKeyID            string                              `json:"signing_key_id,omitempty" jsonschema:"Optional key ID from crypto.keys for per-app JWT signing. Falls back to jwt.preferred_key_id when empty."`
}
